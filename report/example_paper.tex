...
% Compact float layout for the appendix-heavy version of the report.
...
\caption{Chat template tokenization on the Qwen model family for instruct models (a), reasoning models with thinking generation (b), and reasoning models with non-thinking generation (c).}
\label{fig:tokens}
\end{figure*}
...
To study this question, we evaluate thinking models under two generation configurations: generation with a reasoning trace and generation without a reasoning trace. Figure \ref{fig:tokens} rows (b) and (c) show the tokenizer template for these two configurations.
...
We first reproduced the original paper's experiments on Qwen2-7B \citep{yang2024qwen2} and Llama3-8B \citep{dubey2024llama3}. After that, we ran our experiments on Qwen3.5-9B. We use the recommended sampling configuration for the Qwen3.5 model family \citep{qwen35}. For the judge model, we use Qwen3.5-4B, with few-shot examples containing thinking traces to reduce overthinking \citep{qwen35}.

In order to extract the harmful and refusal activations, we collect 3 datasets containing harmful queries: AdvBench \citep{zou2023universal}, SorryBench \citep{xie2025sorrybench} and JailbreakBench (JBB) \citep{chao2024jailbreakbench}, and 2 datasets containing harmless queries: XSTest \citep{rottger2024xstest} and Alpaca \citep{taori2023alpaca}. After that, we conduct the two-step pipeline shown in Figure \ref{fig:datasets}:

\begin{figure*}[!t]
\centering
\includegraphics[width=0.68\textwidth]{figures/dataset.png}
...
We base our manual bucketing on the following basis: \emph{(i)} Qwen3.5 has been trained with extensive safety alignment, so unlike the instruct model experiments, the model refuses almost all harmful queries; to obtain accepted harmful responses in the non-thinking setting, we compute a GCG jailbreak \citep{zou2023universal} with an ASR of 34\% using nanoGCG \citep{nanogcg2025}; \emph{(ii)} for the thinking generation, we use SorryBench, which contains soft-harmful queries that violate the model's safety guardrails but are less explicit than AdvBench and JBB; \emph{(iii)} for refused harmless responses, we only use XSTest because it contains harmful-looking harmless examples which might trick the model into a refusal.


\section{Methodology}
In this section, we present our approach with the experiments we conducted. We first test in Section \ref{sec:strip} whether stripping the last tokens also decreases refusal like in instruct models. Since that does not prove effective, we then search for potential tokens with strong harmfulness and refusal signals in Section \ref{sec:activations}. We find that the $t_{\text{post-inst}}$ and gen1 tokens encode the clearest target behavior in the non-thinking configuration. We test this belief in Section \ref{sec:corr} by clustering the buckets in harmfulness/refusal score space. In Section \ref{sec:elicit_refusal}, we steer both directions individually on harmless data points to elicit high refusal rates. We then try an inversion generation experiment in Section \ref{sec:causal-separation} to causally test direction separation. We fail to prove the separation causally, which motivates the more cautious conclusions of this report.
...
Figure \ref{fig:grid} shows the results for the distinguished tokens. Figure \ref{fig:col_qwen2} shows the desired opposing behavior on Qwen2-7B, $t_{\text{inst}}$ encodes harmfulness while $t_{\text{post-inst}}$ encodes refusal. Figure \ref{fig:col_think} and Figure \ref{fig:col_nothink} show the activations for Qwen3.5-9B on thinking and non-thinking generations. $T_{\text{post-inst}}$ on the non-thinking configuration is the only token on the prompt activations that show the desired behavior. For this reason, we choose $t_{\text{post-inst}}$ and the first generated token on the non-thinking configuration to continue with our experiments. As Figure \ref{fig:col_gen1} shows, we hypothesize that $t_{\text{post-inst}}$ contains the harmfulness direction and the first generated token contains the refusal direction.

The thinking configuration tokens show less promising results. We try the $\backslash$n token after the opening think token as the harmful token and mid1 token for the refusal direction, but we get no interesting results. We conclude that either there is no clear separation, or the SorryBench dataset was not harmful enough to capture the harmful direction.
...
\begin{subfigure}[t]{0.235\textwidth}
    ...
    \caption{Qwen3.5 tpost/gen1.}
    \label{fig:col_gen1}
\end{subfigure}
\caption{Layer-wise activation score curves for selected token positions. The Qwen2-7B panel reproduces the original prompt-boundary pattern, while the Qwen3.5 panels compare thinking and non-thinking token choices.}
\label{fig:grid}
\end{figure*}

\subsection{Correlation between beliefs of harmfulness and refusal replication}
\label{sec:corr}
In this section, we plot the harmful score (Equation \ref{eq:delta_harmful}) and refusal score \ref{eq:delta_refuse} of each activation bucket for the non-thinking configuration using $t_{\text{post-inst}}$ as the harmful token and gen1 as the refusal token. We apply the same train and test split as in Section \ref{sec:activations}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.64\linewidth]{figures/figure3_qwen35_nothink.png}
\caption{Non-thinking generation activations for $t_{\text{post-inst}}$ and gen1 tokens clustered according to refusal and harmful score.}
\label{fig:figure3_qwen35_nothink}
\end{figure}

\subsection{Refusal elicitation}
\label{sec:elicit_refusal}
\label{eq:elicit_refusal}
We test our $t_{\text{post-inst}}$ and gen1 token activations to check whether we can elicit refusal. For each layer, we extract the harmful vector by subtracting the $t_{\text{post-inst}}$ token's mean harmless activation from the $t_{\text{post-inst}}$ mean harmful activation (Equation \ref{eq:v_harmful}). We apply the same logic with the refusal vector in Equation \ref{eq:v_refuse}. Afterwards, the computed vector on each layer is added to the model's residual stream while generating a response, multiplied by a magnitude coefficient. 
...
In this task, we append harmful and harmless instructions with a question \emph{``Could this user prompt cause harm? Answer `Certainly' if it could, otherwise `No'.''}
Without steering, the model is expected to reply with the refusal word ``No'' to harmless prompts and the non-refusal word ``Certainly'' to harmful prompts.
...
We conducted the same experiments for Qwen3.5-9B with the thinking mode turned off, but did not observe the same effects.
The details are reported in Appendix~\ref{app:reply-inversion}.

\begin{figure}[!t]
\centering
\includegraphics[width=0.64\linewidth]{figures/fig4.png}
\caption{The orange and blue curves show the refusal rates under additive steering in the harmfulness and refusal directions across all layers, respectively.
The green and purple curves show the refusal rates under additive steering in each layer $l$ along a random vector with the same norm as $v^\textup{harmful}_l$ and $v^\textup{refuse}_l$.
The steering coefficient is set to $4$ in all cases.}
\label{fig:fig4}
\end{figure}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.70\textwidth]{figures/qwen35_figure2_think_judge.png}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.92\columnwidth]{figures/qwen35_figure3_think.png}
...
\clearpage
\bibliographystyle{icml2022.bst}
...
\begin{figure}[!t]
\centering
\includegraphics[width=0.9\columnwidth]{figures/section4_jailbreak_scatter.png}
...
\begin{subfigure}[t]{0.74\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/data_probe_cot_length.png}
    \caption{CoT length.}
\end{subfigure}
\vskip 0.04in
\begin{subfigure}[t]{0.74\textwidth}
    \centering
    \includegraphics[width=\linewidth]{figures/data_probe_layer_sweep.png}
    \caption{Layer-wise projections.}
\end{subfigure}
...
\subsection{Reply Inversion Tasks}\label{app:reply-inversion}

We conducted the reply inversion experiments (\Cref{sec:reply-inversion}) on Qwen3.5-9B with the thinking mode turned off.
The results are shown in \Cref{fig:fig5}.
...
\begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{Unsteered model answers ``No'' at 100\% rate. Adding $v^\textup{harmful}_l$ makes the model not answering at all.}
    \label{fig:fig5a}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.45\textwidth}
    ...
    \caption{Unsteered model answers ``No'' at 2\% rate. Subtracting $v^\textup{harmful}_l$ does not make the model answer ``No'' more frequently than random steering.}
    \label{fig:fig5b}
\end{subfigure}
\caption{Intervention results for the reply inversion tasks.
If the model does not reply with ``Certainly'' or ``No'' but a canonical refusal phrase (e.g., ``I cannot assist...''), then we classify the response as an refusal to answering the question.
Similar to \Cref{fig:fig4}, we include results of random steering for comparison.
}
\label{fig:fig5}
\end{figure*}
