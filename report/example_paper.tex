...
\section{Methodology}
In this section, we present our approach with the experiments we conducted. We first test in Section \ref{sec:strip} whether stripping the last tokens also decreases refusal like in Instruct models. Since that proves not effective, we then search for potential tokens with a strong harmful and refusal signal on Section \ref{sec:activations}. We find that the tpost and gen1 tokens encode our desired behaviour on the non-thinking configuration. We confirm this belief in Section \ref{sec:corr}, clustering the buckets in a harmful and refusal score space. In Section \ref{sec:elicit_refusal} then we steer both directions individualy on harmless data points to elicit high refusal rates. We then try an inversion generation experiment in Section (insert cite) to causaly prove the direction separation. We fail to prove the separation, hence our conclusions. 
...
Figure \ref{fig:figure3_qwen35_nothink} shows the resulting clusters, the four clusters are clearly defined on it's corresponging space. We note that the magnitude of the score is nearly half as the replication for the Qwen2 model, suggesting that although both directions are present, the directions might not be as distinct as expected.
...
We test our tinst and gen1 tokens activations to check whether we can elicit refusal. For each layer, we extract the harmful vector substracting the tpost token's mean harmless activation from the tpost mean harmful activation (Equation \ref{eq:v_harmful}). We apply the same logic with the refusal vector in Equation \ref{eq:v_refuse}. Afterwards, the computed vector on each layer is added to the model's residual stream while generating a response, multiplied by a magnitude coefficient. 
...
Figure~\ref{fig:fig4} shows that steering the model in the harmfulness or refusal directions can elicit refusal behavior on harmless instructions.
To disentangle the refusal behavior from the model's internal judgement of harmfulness, we follow \citet{zhao2025harmfulness} to perform a reply inversion task.
...
Conversely, steering into the harmless direction would result in the model classifying harmful instructions as not harmful.
\citet{zhao2025harmfulness} observed this change of behavior in the instruct model Qwen2-7B and claimed it as an evidence for the harmfulness and refusal concepts to be causally separable.
